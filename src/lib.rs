...
impl SocketAddr {
    ...
    pub fn new_ip6_port(a0: u16, a1: u16, a2: u16, a3: u16,
                        a4: u16, a5: u16, a6: u16, a7: u16,
                        port: u16) -> Self {
        SocketAddr::V6(SocketAddrV6::new_ip6_port(a0, a1, a2, a3, a4, a5, a6, a7, port))
    }
...
mod ipv4 {
    ...
    use super::{Error, port_to_bytes, Result};
    ...
    impl SocketAddrV4 {
        ...
        pub fn new_ip4_port(a0: u8, a1: u8, a2: u8, a3: u8, port: u16) -> Self {
            SocketAddrV4 { addr: Ipv4Address::new(a0, a1, a2, a3), port }
        }
...
mod ipv6 {
    ...
    pub fn ipv6_addr(a0: u16, a1: u16, a2: u16, a3: u16,
                     a4: u16, a5: u16, a6: u16, a7: u16) -> Ipv6Address {
    ...
    impl SocketAddrV6 {
        ...
        pub fn new_ip6_port(a0: u16, a1: u16, a2: u16, a3: u16,
                            a4: u16, a5: u16, a6: u16, a7: u16,
                            port: u16) -> Self {
            SocketAddrV6 { addr: Ipv6Address::new(a0, a1, a2, a3, a4, a5, a6, a7), port }
        }
...
mod std {
    ...
    impl From<SocketAddr> for StdSocketAddr {
        fn from(val: SocketAddr) -> Self {
            match val {
                SocketAddr::V4(val) => StdSocketAddr::V4(StdSocketAddrV4::new(val.addr.0.into(), val.port)),
                SocketAddr::V6(val) => StdSocketAddr::V6(StdSocketAddrV6::new(val.addr.0.into(), val.port, 0, 0)),
            }
        }
    }
    ...
    impl From<StdSocketAddr> for SocketAddr {
        fn from(val: StdSocketAddr) -> Self {
            match val {
                StdSocketAddr::V4(val) => SocketAddr::V4(SocketAddrV4 { addr: Ipv4Address::from(*val.ip()), port: val.port() }),
                StdSocketAddr::V6(val) => SocketAddr::V6(SocketAddrV6 { addr: Ipv6Address::from(*val.ip()), port: val.port() }),
            }
        }
    }
}
...
mod tests {
    ...
    fn ipv4_socket_addr() {
        ...
        assert_eq!(socket_addr.addr.as_bytes(), &socket_addr.to_vec().as_slice()[0..4]);
    ...
    fn ipv6_socket_addr() {
        ...
        assert_eq!(socket_addr.addr.as_bytes(), &ip_address_bytes);
        assert_eq!(socket_addr.addr.as_bytes(), &socket_addr.to_vec().as_slice()[0..16]);
        assert_eq!(socket_addr.port, 8080);
    }
    ...
    fn ipv4_to_std() {
        ...
        assert_eq!(Ok(socket_addr), SocketAddr::new(IpAddress::Ipv4(ip_address), 80));
        assert_eq!(Err(Error::UnspecifiedIp), SocketAddr::new(IpAddress::Unspecified, 80));
    ...
    fn ipv6_to_std() {
        ...
        assert_eq!(std_ip_address, Ok(IpAddr::V6(Ipv6Addr::new(0, 0, 0, 0, 0, 0, 0, 1))));
        ...
        assert_eq!(Ok(socket_addr), SocketAddr::new(IpAddress::Ipv6(ip_address), 80));
        assert_eq!(Err(Error::UnspecifiedIp), SocketAddr::new(IpAddress::Unspecified, 80));
        assert_eq!(socket_addr, SocketAddr::new_v6(ip_address, 80));
        assert_eq!(socket_addr, SocketAddr::new_ip6_port(0, 0, 0, 0, 0, 0, 0, 1, 80));
        assert_eq!(socket_addr.len(), 18);
        assert_eq!(socket_addr.ip().as_bytes(), ip_address_bytes);
        assert_eq!(socket_addr.ip_octets(), ip_address_bytes);
        let socket_addr_vec = socket_addr.to_vec();
        assert_eq!(socket_addr_vec[0..16], ip_address_bytes);
        assert_eq!(port_from_bytes(socket_addr_vec[16], socket_addr_vec[17]), 80);
        info!("socket_addr: {}", socket_addr);
        info!("socket_addr debug: {:?}", socket_addr);
